    ...
    public function store()
    {
        if (isset($_POST['add'])) {
            ...
            if ($title == '' || $url == '' || $_FILES['file']['name'] == '') {
                Helper::flash('errors', 'Vui lòng nhập Tiêu đề, Đường dẫn và Ảnh');
    ...
    public function update($id = 0)
    {
        if (isset($_POST['update'])) {
            ...
            if ($slider) {
                ...
                #Nếu người dùng đã chọn file Ảnh thì mới kiểm tra
                if ($_FILES['file']['name'] != '') {
                    #Kiểm tra định dạng Ảnh
                    $check = getimagesize($_FILES["file"]["tmp_name"]);
                    if (!$check) {
                ...
                if ($result) {
                    Helper::flash('success', 'Cập nhật Slider thành công');
                    return Helper::redirect('admin/slider/lists');
                }
